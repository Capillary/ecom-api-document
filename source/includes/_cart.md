...
ShipState": "KW2",
DelveryMode": "H",
ProductID  | int  |   13047461,
VariantProductID  | int |   0,
Quantity  | int |   1,
Status  | enum |   A,
LocationId  | int |   27299,
Portion  | enum |   `W` for whole, For toppings `L` left, Right R,
CartReferenceKey  | string |   
Price  | float |   3.25,
ChildItem  | json obj |   All add--ons are child in a bundle. For example, crust, sauce, and toppings
GroupID  |  |   13317, One group can have only one type of item. For example, in a group you can either add only pizzas, drinks, appetizers or deserts.
...
ChildItem | - | 
ProductID | int | 13047239,
VariantProductID | int | 9757649,
Quantity | int | 1
Status | enum | A
Portion | enum | W
GroupID |  | 13245 
...
activestatus | boolen | Specify `true` to fetch the list of only active promotions, `false` to fetch both active and inactive coupons
channeltype |  | 
