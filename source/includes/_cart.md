...
Lets you add customized bundle items to cart.
...
Portion | enum | The portion for which the child product is applied. Value: `W` for whole, `R` for right, `L` for left (left, right usually applies for toppings)
GroupID | int | Group id of the specific item. Applicable in case of a bundle or deal products
...
OrderAttributes | json obj | Provide the details of order attributes that you want to add/update in the parameters explained below
...
IsPredefined | boolean | Specify `true` if the order attribute are predefined in the Control Panel, specify `false` for free-flow values
...
lstOMSEntityFieldValues | string | For predefined order attribute values (`IsPredefined` is true) provide the attribute values as per options configured on Control Panel 
...
BillingAddress | json obj | Specify the billing address of the customer in the respective attributes
ShippingAddress | json obj | Specify the preferred shipping address of the customer in the respective attributes
BillMobile | string | Mobile number associated to billing address
...
BillCountryCode | string | Alpha-2 code of the country with respect to billing address. Example: IN (for India), AU (for Australia), and BR (for Brazil) 
BillLastName | string | Customer's last name associated to the billing address 
BillState | string | Full name of the state associated to the billing address
BillAddress | string | Billing address - door number, building name, street, area and so on
...
------- | ------ | ---------
merchantId* | string | Unique GUID of the merchant 
voucherCode* | string | The voucher code that has to be removed from the cart
...
Parameter | Type | Description
--------- | ---- | ----------
locationcode | string | The location code of the store to fetch the offers associated to the specific store
deliverymode | enum | Filter results (promotions) by delivery mode. Values: `ship` for promotions on online orders, `pickup` for promotions on store pickup orders
...
hasvouchers | boolean | Specify `true` to get only the list of promotions having coupon codes, `false` to get only promotions without coupon codes. You can leave it blank to see both kind of promotions
...
Parameter | Type | Description
-------- | ----- | -----------
DeliverySlotId* | int | Unique id of the delivery slot to which the customer wants the current cart items delivered
...
Retrieves all the available delivery slots of the cart items
